...
var checkNotInputAmount = (className) => {
  ...
  $(className).each(function () {
    if ($(this).val() <= 0) {
      ...
      $(this).after(
        `<span class="error invalid-feedback" style="display:block">Chưa nhập giá trị</span>`,
      );
      check = false;
    }
  });
...
var dialog = (function () {
  return {
    ...
    show: function (title, data) {
      ...
      $('#btnAction').attr(
        'onclick',
        'return formHelper.onSubmit("frmDialogCreate")',
      );
      $('#detailModal').modal('show');
    },
    close: function () {
...
var formHelper = (function () {
  return {
    postFormJson: function (objectId, onSuccess, onError) {
      var url = document.getElementById(objectId).action;
      $.post(
        url,
        $('#' + objectId).serialize(),
        function (data) {
...
var renderError = (errors) => {
  for (var key in errors) {
    ...
    $(`[name=${key}]`).after(
      `<span class="error invalid-feedback" style="display:block">${errors[key]}</span>`,
    );
  }
};

const renderErrorFormException = (errors) => {
  for (var key in errors) {
    if (typeof errors[key].errors == 'string') {
      $(`[name=${key}]`).addClass('is-invalid');
      $(`[name=${key}]`).next('.invalid-feedback').remove();
      $(`[name=${key}]`).after(
        `<span class="error invalid-feedback" style="display:block">${errors[key].errors}</span>`,
      );
    }
  }
};
    ...
    upload: function (
      ...
      $.ajax({
        ...
        success: (response) => {
          if (response) {
            if (multiple) {
              let dataListUrl = $(idUrlFile).val().length
                ? JSON.parse($(idUrlFile).val())
                : [];
              dataListUrl.push(response.file);
              $(idUrlFile).val(JSON.stringify(dataListUrl));
            } else {
...
$(document).ready(function () {
  $(document).on('keyup', '.number', function (e) {
    var val = helper.removeCommas($(this).val());
    $(this).val(helper.addCommas(val));
  });
  ...
  $('.js-checkbox-groups').click(function (e) {
    ...
    if ($(this).prev().is(':checked') === true) {
      $(this)
        .closest('.card')
        .find('.card-header input:checkbox')
        .prop('checked', false);
      $(this)
        .closest('.card')
  ...
  $('.js-checkbox-single').click(function (e) {
    ...
    checkboxAll(
      $(this).closest('.card').find('.card-body input:checkbox'),
      $(this).closest('.card').find('.card-header input:checkbox'),
    );
  });
  /////////////////////END ROLES//////////////////////////////
  ...
  $('.js-checkbox-list-single').click(function (e) {
    ...
    checkboxAll(
      ...
      $(this).closest('table').find('thead input:checkbox'),
    );
  });
  ...
  $('.js-edit-record').click(function (e) {
    ...
    $($('table').find('tbody .icheck-info input:checkbox')).each(function (
      index,
      element,
    ) {
  ...
  $('.js_select_multi_record_patient').click(function (e) {
    ...
    let id = [];
    let url = $(this).data('url');
    $($('.md-table-content').find('ul li .icheck-info input:checkbox')).each(
  ...
  $('.add_member_button').click(function (e) {
    ...
    let item =
      ...
      '][name]"\
                ...
                <div class="col-5">\
                    <input type="text" class="form-control datepicker" name="members[' +
      key +
      '][birthday]"\
